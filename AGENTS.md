...
This repository is a Go module (`module notes`) with most implementation under `code/aggregate_registry/`. Root-level `.md` files are design and requirements notes. Inside this module, packages are organized by responsibility: `contract/`, `config/`, `dispatch/`, `delivery/`, `consumer/`, `render/`, `scheduler/`, `publisher/`, `preview/`, `bootstrap/`, and `handlers/<message_type>/`. Static templates live in `templates/`, and sample inputs/outputs are stored as JSON alongside the code.

## Build, Test, and Development Commands
- `go test ./...` runs the full test suite from the repository root.
- `go build ./...` verifies all packages compile.
- `cd code/aggregate_registry; go test ./preview` runs the preview-focused package tests.
...
## Agent Notes
Treat `code/aggregate_registry/README.md` and the package tests as the source of truth for current behavior. Avoid adding new abstractions unless they reduce complexity in the existing dispatch/render pipeline.
