...
1. 汇总类消息生成方式最终采用方案一还是方案二。
2. `severity`、资产范围以及更复杂筛选条件，是否纳入通知模块配置域。
3. 批次表是否最终确定为两张表，还是退化为单表。
4. 批次负载是否直接落 `payload_snapshot`，还是引入 `payload_ref` 做大对象旁路。
5. `notification HTTP` 的统一请求模型如何抽象公共字段。
6. 幂等 key 是否最终纳入 `channel + configId` 维度。
7. 失败补发策略的默认次数、间隔和人工介入方式如何设定。
8. 租户维度配置缓存的 TTL 是否固定为 `3m`，是否需要手动刷新能力。
9. 监控指标和告警阈值的最小上线要求是什么。
10. 是否默认启用通知模块统一 `1s` 微批写入，以及批量条数阈值如何设定。
11. 如果采用 `OceanBase` 作为待发送事实源，分区粒度、结果保留周期以及历史分区清理机制如何定稿。
...
- 方案一中落 producer sdk，或在方案二中定稿业务标准查询接口接入规范
- 落双通道批次扫描
- 落模板渲染和 `notification client`
- 落状态回写和有限重试

输出物：

- producer sdk 或标准接口接入规范
- `aes-crontab` 执行器能力
- 调用 `notification` 的统一 client
...
灰度策略上，建议优先按租户粒度放量。原因是当前通知治理本身就是租户级生效配置，灰度影响面天然较小，更适合先验证：

- 限流返回是否完整
- 上游是否能正确识别并退避
- 个别租户放量后是否对整体链路造成额外抖动

## 验收标准

方案进入实现前，至少应满足以下验收标准：

- 目标清晰：评审结论明确本次方案围绕通知能力主体建设展开
- 协议统一：方案一所需的上游接入协议或方案二所需的业务接口契约，与 `notification` 调用协议一并定稿
- 配置闭环：默认配置、租户覆盖、最终生效配置链路闭环
- 执行闭环：批次生成、扫描发送、状态回写、有限重试闭环
- 观测闭环：关键指标、日志字段、基础告警闭环
- 灰度闭环：能够支持小范围租户或消息类型灰度上线
