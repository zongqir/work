...
- `stage` 取值收敛为 `query` / `materialize`

这里 `materialize` 指“已经从业务接口拿到结果，并成功或失败地形成待发送批次”，不直接覆盖后续 `notification` 发送结果。发送侧是否成功，仍然以下游批次状态为准。
