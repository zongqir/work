...
- 执行时已经明确 `tenant_id` 和 `message_type`
- AES 按接口向 Apex 获取该租户该消息类型的最终生效策略
- AES 本地读取 `template_code` 等模板配置
- 为减少频繁读取，对 Apex 返回的租户生效策略做本地短 TTL 缓存，例如 `3m`
- TTL 过期后重新读取并刷新缓存

这里的规则很简单：

- Apex 侧负责租户策略合并，AES 不关心其内部默认值和租户差异如何组织
- AES 侧负责模板配置管理，不把 `template_code` 下沉给 Apex
- 读取频率通过本地 TTL 控制
...
- 模型更轻，不需要维护全量租户快照
- 执行器只关心当前请求对应的最终执行配置
- 租户策略合并逻辑保留在 Apex 内部，AES 只关心接口契约、读取成本和本地模板配置
