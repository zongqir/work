...
- [3. 模块详细设计](#3-模块详细设计)
  - [3.1. 触发边界](#31-触发边界)
  - [3.2. 实时筛选承接方式](#32-实时筛选承接方式)
  - [3.3. 配置模型](#33-配置模型)
  - [3.4. 配置生效](#34-配置生效)
  - [3.5. 频率控制与发送时机](#35-频率控制与发送时机)
  - [3.6. 批次模型](#36-批次模型)
  - [3.7. 冻结参数原则](#37-冻结参数原则)
  - [3.8. 执行模型](#38-执行模型)
  - [3.9. 状态设计](#39-状态设计)
  - [3.10. 幂等设计](#310-幂等设计)
  - [3.11. 与 Notification 的接口边界](#311-与-notification-的接口边界)
  - [3.12. 数据模型](#312-数据模型)
  - [3.13. 可靠性与可观测性](#313-可靠性与可观测性)
- [4. 关联分析](#4-关联分析)
  - [4.1. 外部依赖关联](#41-外部依赖关联)
- [5. 变更控制](#5-变更控制)
  - [5.1. 变更列表](#51-变更列表)
- [6. 附录](#6-附录)
  - [6.1. 关键状态枚举](#61-关键状态枚举)
  - [6.2. 幂等 Key 示例](#62-幂等-key-示例)
- [7. 修订记录](#7-修订记录)
...
|-----------|------|
| APEX | 底座配置与租户生效策略提供方 |
| OB | OceanBase，用于承接待发送批次或 `outbox` 数据 |
| sender | 上游调用入口，负责把待发送内容写入 `OB` |
| effective_policy | 按 `tenant_id + message_type` 计算后的最终生效策略 |
| realtime batch | 实时通知待发送批次 |
| schedule batch | 聚合/定时通知待发送批次 |
| skip | 明确判定不发送的终态，不进入真实渠道下发 |
| notification | 下游统一发送底座，负责具体渠道下发 |

### 1.3. 参考和引用

- [格式.md](/C:/Users/Administrator/code/notes/格式.md)
- [AES总设.md](/C:/Users/Administrator/code/notes/AES总设.md)
- [多租户HTTP调度详细设计.md](/C:/Users/Administrator/code/notes/多租户HTTP调度详细设计.md)
- [AES共享ConfigMap动态生效设计.md](/C:/Users/Administrator/code/notes/AES共享ConfigMap动态生效设计.md)
...
- 汇总类消息生成方式设计
- 待发送结果承接与发送治理
- Apex 底座统一承接租户侧通知策略存储与生效配置计算，AES 侧消费其策略接口并自行管理模板配置
...
- 通知能力采用“统一通知模块承接配置、执行和治理”的方案
- 发送底座继续复用现有 `notification HTTP`
- 租户侧通知策略由 Apex 统一存储并计算最终生效策略
- 执行层优先复用 `aes-crontab` 承载，不新增独立通知执行微服务
- 实时链路采用“入口轻量过滤 + `OB` 出队前二次判定”的方式控制无效写入
...
- 汇总类消息究竟由业务侧产出结果，还是由通知模块按周期拉取业务接口，属于本次待评审的决策
- 底座继续承接具体渠道发送
...
- 聚合调度任务：按配置和时间窗口判断哪些聚合任务当前需要执行，调用业务标准接口取数，并生成 `aes_notify_schedule_batch`
- 实时发送任务：持续扫描 `aes_notify_realtime_batch`，按实时模板执行发送
- 定时发送任务：持续扫描 `aes_notify_schedule_batch`，按聚合模板执行发送

其中：

- 聚合调度任务只负责产出 `schedule batch`，不直接调用 `notification`
- 所有真实发送动作统一收敛到发送任务内完成

这样拆分的原因是：

- 实时通知和聚合通知天然使用不同模板
- 实时链路更强调低延迟，聚合链路更强调吞吐和稳定
- 发送表既然已经拆分，执行任务继续拆分会更容易保持职责清晰
- 虽然执行任务拆成三类，但 `notification client`、幂等规则、失败回写和观测口径仍然统一

#### 2.6.4. Notification 底座
...
无论采用哪种方案，通知模块（AES封装+NOTIFICATION共同承载）都至少会承接以下执行类配置：
...
#### 3.2.2. 方案 B：先转标准化事实模型，再由通知模块统一筛选
...
- 租户侧通知策略由 Apex 底座统一承接，负责存储、默认值与租户差异合并，以及最终生效策略计算
- AES 自身保留模板配置，尤其是 `template_code` 和模板内容资产
- AES 运行时通过接口获取 Apex 的租户生效策略，再与本地模板配置组合成最终执行配置
...
考虑到实时场景上游写入量可能很大，执行层不应把“实时写入”卡在 Apex 配置读取上，否则一旦 Apex 抖动，就会直接影响实时结果入链路。
所以第一步“后台定期同步配置”：

- 让实时链路在入口处就具备一层轻量的本地判定能力
- 尽量把明显不该进入发送链路的数据挡在 `OB` 之前
- 只把“看起来可能需要发送”的数据写入 `OB`

把配置生效成下面两层机制：

1. 后台定期同步

- AES 后台任务定期同步Apex 最新租户生效策略
- 同步结果落到本地缓存，按 `tenant_id + message_type -> effective_policy` 组织
- 实时链路优先使用这份本地缓存做入口快速过滤（大部分是没开的），避免所有消息都直接写入 `OB`

2. 发送前二次判定

- 实时链路收到待发送结果后，先根据本地缓存做一次轻量判断
- 如果本地配置已明确判定“该消息类型未启用”或“当前不满足发送条件”，则入口直接跳过，不写 `OB`
- 如果本地配置判断“可能需要发送”，则调用 sender，由 sender 先把数据写入 `OB`
- 如果当前本地拿不到配置，为避免主链路阻塞和丢失，先写入 `OB`
- `OB` 后续真正发送前，再读取当前本地配置并判断是否允许发送
- 如果当前配置判定“允许发送”，则继续正常发送
- 如果当前配置判定“该消息类型未启用”或“当前不满足发送条件”，则不真正下发渠道，而是把该条记录直接标记为 `skip`

这里的关键是：

- 实时写入链路和配置读取链路解耦
- 不要求每条实时消息都同步读取 Apex
- 后台同步配置的首要价值之一，是减少无效 `OB` 写入
- 能不能发，以真正发送前看到的最新配置为准
- 明确不能发的记录，不算失败，直接进入 `skip` 终态

好处是：

- 实时主链路更轻，不会因为配置中心短时抖动而卡住写入
- 大量明显不需要发送的数据可以在入口被挡掉，不进入 `OB`
- 发送治理统一收口在 `OB` 出队阶段，边界更清楚

运行期只需要维护租户维度的生效策略缓存：

- `tenant_id + message_type -> effective_policy`

其中 `effective_policy` 由 Apex 提供，执行链路真正消费的是“Apex 租户生效策略 + AES 模板配置”组合后的最终执行配置。
...
如果最终采用方案二，即通知模块统一按窗口周期调用业务标准接口，那么首先需要把“请求协议”收敛住。

这里先只约定请求，不在本节展开返回结果定义。

请求协议遵循以下原则：

- 请求只表达“本次要聚合哪个租户、哪个时间窗口、使用哪份业务配置”
- 不把分页、游标、条数限制等执行细节放进聚合请求
- 不把 `rule_id`、`trigger_at` 这类运行态元数据放进聚合请求主体
- 不试图在通知模块里统一筛选语义，筛选条件由业务配置自己解释

因此，方案二下聚合接口的最小请求语义收敛为：
...
|------|------|
| `tenant_id` | 本次聚合所属租户 |
| `window_start` | 本次聚合窗口起点，含边界 |
| `window_end` | 本次聚合窗口终点，不含边界 |
| `config_body` | 业务聚合配置，由具体业务接口自行解释，例如筛选条件、排序方式、样例条数、分组口径等 |
...
- 同一个窗口是否取 `window_start <= data_time < window_end`，需要在协议中统一固定，避免不同业务各自理解
- `config_body` 可以不统一字段名，但必须保证同一业务接口内语义稳定、可校验
- 聚合实现方如果内部需要分批扫描、分段查询或多次聚合，属于实现细节，不体现在接口请求中
- 聚合器选择应由 AES 本地配置或接口路由关系决定，不依赖在请求体里额外塞一个聚合类型字段

也就是说，通知模块统一的是“窗口执行协议”，不是“筛选字段模型”。

#### 3.5.4. 补充：窗口执行记录

如果最终采用方案二，即通知模块统一按周期调用业务标准接口，再生成待发送结果，那么建议补一层轻量的窗口执行记录。

原因很实际：

- 聚合本质上是按固定窗口触发的后台执行，而不是单条事件即时发送
- 仅依赖待发送批次，无法稳定区分“本窗口未执行”“执行了但查询失败”“执行成功但空结果”“已经生成批次但发送失败”这几类情况
- 没有执行记录时，后续补跑、排障、对账和窗口级观测都会比较被动

这里的执行记录只回答一个问题：

- 某个 `config_id` 在某个 `window_start ~ window_end` 上，当前执行到了哪一步，为什么没有产出或没有发出

默认执行原则也一并收敛如下：

- 方案二默认只执行当前调度对应的目标窗口，执行记录仅用于标记该窗口是否成功，不依据历史执行缺口自动补齐；历史补跑如有需要，作为单独运维动作处理

建议执行记录只服务于方案二的“窗口取数与结果生成”，不与发送批次职责混在一起：

- 执行记录负责描述窗口级取数与产物生成结果
- 批次表负责描述具体待发送内容及其发送重试状态



落地时遵循以下约束：

- 同一 `config_id + window_start + window_end` 只允许存在一条活跃执行记录
- 窗口调度触发后，先尝试创建或抢占这条执行记录，再调用业务接口
- 查询失败时，更新执行记录为 `failed`，并记录 `stage=query`
- 查询成功但空结果时，执行记录可直接记为 `success`，同时 `result_count=0`
- 查询成功且有结果时，先生成待发送批次，再把执行记录记为 `success`，并关联 `batch_id`
- 后续如果批次发送失败，不回头修改窗口执行记录，而是在批次表内继续按发送失败逻辑补发

这样拆分后，方案二的链路会更清楚：

- 执行记录解决“这个窗口有没有跑、为什么没产出”
- 批次记录解决“这批待发送内容有没有成功送到 `notification`”
...
- 实时消息和聚合消息天然对应不同模板，拆表后模板选择和发送策略都更清晰

模板与批次表的对应关系也一并明确：

- `aes_notify_realtime_batch` 走实时模板
- `aes_notify_schedule_batch` 走聚合模板

因此，方案二下的执行链路收敛为：

- 聚合调度任务负责生成 `schedule batch`
- 实时发送任务负责消费 `realtime batch`
- 定时发送任务负责消费 `schedule batch`
...
配置存储不在 AES 内部展开数据表设计，统一由 Apex 承接。

AES 与 Apex 之间只需要定清以下内容：

- 查询接口入参，例如 `tenant_id + message_type`
- 返回的租户生效策略字段，例如 `enabled`、`channels`、`receivers`
- 配置动态生效时延要求
- 接口可用性、超时、降级和缓存策略

至于默认值如何组织、租户差异如何存储、内部是否拆表或做覆盖模型，均由 Apex 自行定稿，不在本方案内展开；`template_code` 和模板内容仍由 AES 自己维护。
...
- 与 Apex 的关联：
  - Apex 负责租户侧通知策略存储与最终生效策略计算
  - AES 通过接口消费 `effective_policy`，不依赖 Apex 内部表结构
- 与 `notification` 的关联：
  - `notification` 负责具体渠道下发与下游幂等兜底
  - AES 负责上游承接、执行治理、状态回写和失败补发
- 与 `aes-crontab` 的关联：
  - `aes-crontab` 作为当前第一版通知执行器承载体
  - 负责扫描批次、调度发送、失败补发和执行观测
...
- **Apex 统一承接租户侧通知策略存储与最终生效策略计算，AES 按接口获取策略并自行维护模板配置**
...
- 租户侧通知策略收口到 Apex，更符合底座职责
- AES 不必绑定 Apex 的具体存储模型，只需要与其把策略接口契约定清
...
- `pending`：已进入待发送链路，等待执行
- `success`：已成功发送
- `failed`：本轮执行失败，可继续补发
- `terminal_failed`：超过补发上限后的终态失败
- `skip`：根据当前配置明确判定不发送
...
| 版本 | 日期 | 修订内容 |
|------|------|----------|
| v0.1 | 2026-04-27 | 按 `格式.md` 统一目录、编号层级和章节结构，并整理配置生效相关结论 |
